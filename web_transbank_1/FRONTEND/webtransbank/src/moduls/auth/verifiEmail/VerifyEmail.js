...
function VerityEmail({email, getUser, sendUserKey}) {
  ...
  const sendCode = (e) => {
    ...
    api.post('/pagina/verify-code/',
      body,
      {headers: {'content-type': 'application/json'}}
    ).then(res=>{
      if (res && res.data && res.data.keyuser){
        const keyuser = res.data.keyuser;
        localStorage.setItem('userkey', keyuser)
        getUser();
        sendUserKey(res.data.keyuser);
      }
    }).catch(err=>{
...
export default connect(null, { getUser })(VerityEmail)